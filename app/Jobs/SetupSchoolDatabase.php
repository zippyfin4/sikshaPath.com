    ...
    public function handle(
        ...
        try {
            ...
            if ($this->packageId) {
                ...
                $cache->removSiksaPathCache(config('constants.CACHE.SCHOOL.SETTINGS'), $school->id);
            }
            ...
            Mail::send('schools.email', $data, static function ($message) use ($data, $settings) {
                $message->to($data['email'])->from($settings['mail_username'] ?? 'SiksaPath Saas')->subject($data['subject']);
            });
