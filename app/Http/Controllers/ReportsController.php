...
class ReportsController extends Controller
{
    ...
    private function getOnlineExamResults($studentId, $sessionYearId)
    {
        ...
        $attempts = $this->studentOnlineExamStatus->builder()
            ->with([
                ...
                'online_exam.class_subject' => function ($q) {
                    $q->withTrashed();
                },
                'online_exam.class_subject.subject'
            ])
            ->where([
                'status' => 2,
        ...
        foreach ($attempts as $student_attempt) {
            ...
            $examRow['id'] = $student_attempt->online_exam_id;
            $examRow['exam_title'] = $student_attempt->online_exam->title;
            $examRow['name'] = $student_attempt->online_exam->title;
            $examRow['description'] = $student_attempt->online_exam->description ?? '';
            $examRow['subject_name'] = $student_attempt->online_exam->class_subject->subject->name ?? 'General';
            ...
            /* ================= CHECK NEW EVALUATION DATA ================= */

            $studentAnswersBaseQuery = $this->onlineExamStudentAnswer->builder()
                ->where([
                    'student_id' => $student_attempt->student_id,
                    'online_exam_id' => $student_attempt->online_exam_id,
                ]);

            $hasNewEvaluationData = (clone $studentAnswersBaseQuery)
                ->whereNotNull('true_answer')
                ->whereNotNull('question_marks')
                ->exists();

            /* ================= MARKS CALCULATION ================= */

            if ($hasNewEvaluationData) {

                // ✅ NEW LOGIC (SOURCE OF TRUTH)

                $total_obtained_marks = $this->onlineExamStudentAnswer->builder()
                    ->where([
                        'student_id' => $student_attempt->student_id,
                        'online_exam_id' => $student_attempt->online_exam_id,
                        'true_answer' => 1
                ...
                $total_marks = $this->onlineExamStudentAnswer->builder()
                    ->where([
                        'student_id' => $student_attempt->student_id,
                        'online_exam_id' => $student_attempt->online_exam_id,
                    ])
                ...
                // ⚠️ FALLBACK — OLD LOGIC (LEGACY)

                $submittedQuestionIds = $this->onlineExamStudentAnswer->builder()
                    ->where([
                        'student_id' => $student_attempt->student_id,
                        'online_exam_id' => $student_attempt->online_exam_id
                    ])
                ...
                $questionIds = $this->onlineExamQuestionChoice->builder()
                    ->whereIn('id', $submittedQuestionIds)
                    ->pluck('question_id');

                $attemptedOptionIds = $this->onlineExamStudentAnswer->builder()
                    ->where([
                        'student_id' => $student_attempt->student_id,
                        'online_exam_id' => $student_attempt->online_exam_id
                    ])
                ...
                foreach ($questionIds as $questionId) {
                    ...
                    $hasWrongAnswer = $this->onlineExamQuestionOption->builder()
                        ->where([
                            'question_id' => $questionId,
                            'is_answer' => 1
                        ])
                        ->whereNotIn('id', $attemptedOptionIds)
                        ->exists();

                    if ($hasWrongAnswer) {
                        unset($questionIds[array_search($questionId, $questionIds->toArray())]);
                    }
                }

                $correctQuestionIds = $this->onlineExamQuestionOption->builder()
                    ->where([
                        'is_answer' => 1
                    ])
                    ->whereIn('id', $attemptedOptionIds)
                    ->whereIn('question_id', $questionIds)
                    ->pluck('question_id');

                $total_obtained_marks = $this->onlineExamQuestionChoice->builder()
                    ->where('online_exam_id', $student_attempt->online_exam_id)
                    ->whereIn('question_id', $correctQuestionIds)
                    ->sum('marks');
            ...
            $total_marks = $total_marks > 0 ? $total_marks : 1;
            ...
            $examRow['percentage'] = $percentage;
            $examRow['status'] = $total_obtained_marks >= ($student_attempt->online_exam->passing_marks ?? ($total_marks * 0.33)) ? 'Pass' : 'Fail';
            $examRow['exam_type'] = 'Online Exam';
            $examRow['created_at'] = $student_attempt->created_at;
            ...
            $examRow['summary'] = [
                ...
                'division' => $this->getDivisionByPercentage($percentage),
                'rank' => 'N/A'
            ];
    ...
    public function exam_reports()
    {
        ...
        $classSections = $this->classSection->builder()->with('class', 'class.shift', 'section', 'medium')->get();
